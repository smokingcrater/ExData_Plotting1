...
generate_plot4 <- function(file_raw_data, date_start, date_end, file_output)
{
	# Read the data.
	...
	par(mfrow = c(2,2))
	plot(df_epc[,"date_time"], df_epc[,"global_active_power"], type="l", xlab="", ylab="Global Active Power")
	plot(df_epc[,"date_time"], df_epc[,"voltage"], type="l", xlab="datetime", ylab="Voltage")
	...
	plot(df_epc[,"date_time"], df_epc[,"global_reactive_power"], type="l", xlab="datetime", ylab="Global_reactive_power")
	dev.off()
}

generate_plot4("household_power_consumption.txt", "1/2/2007", "3/2/2007", "plot4.png")
